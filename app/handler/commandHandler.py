...
from app.server.fileServer import no_keyword, has_keyword, construct_file, get_file_by_part_url
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if context.args:
        ...
        full_url = get_file_by_part_url(context.args[0].replace('lee7s', '.'), str(update.effective_chat.id))

        logging.info(f"用户 {update.message.chat.username} 下载了文件")

        await context.bot.delete_message(update.effective_chat.id, update.effective_message.id + 1)
        if full_url == "":
            return await context.bot.send_message(chat_id=update.effective_chat.id, text="文件下载失败 请重试")

        try:
            return await context.bot.send_document(chat_id=update.effective_chat.id,
                                            document=full_url)
...
async def get_file_list(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for file in file_list:
        result_list.append(
            f"{number} -- <a href='https://t.me/lee7s_bot?start={file.file.split('/')[2].replace('.', 'lee7s')}'>{file.fileName}</a>")
